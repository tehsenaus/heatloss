...
var $author$project$Main$decodeParams = function (floats) {
	...
	return {
		...
		householdElecKwh: A2(getS, 29, d.householdElecKwh),
...
var $author$project$Main$encodeParams = function (m) {
	return _List_fromArray(
		[
			...
			$author$project$Main$toF(m.summerTempOut),
		...
		switch (msg.$) {
			...
			case 'SetTempOut':
				...
				return _Utils_update(
					model,
					{tempOut: v});
			case 'SetHeatedFloorArea':
				var v = msg.a;
				return _Utils_update(
					model,
					{heatedFloorArea: v});
			case 'SetFloorCovering':
				var c = msg.a;
				return _Utils_update(
					model,
					{floorCovering: c});
			...
			case 'SetThermalMass':
				...
				return _Utils_update(
					model,
					{thermalMass: t});
			case 'SetSummerTempOut':
				var v = msg.a;
				return _Utils_update(
					model,
					{summerTempOut: v});
			case 'SetHouseholdElecKwh':
				var v = msg.a;
				return _Utils_update(
					model,
					{householdElecKwh: v});
			case 'SetEvMilesPerYear':
				var v = msg.a;
				return _Utils_update(
					model,
					{evMilesPerYear: v});
...
var $author$project$Main$effectiveACH = function (m) {
	var _v0 = m.ventMode;
	if (_v0.$ === 'DirectACH') {
		return $elm$core$String$toFloat(m.ach);
	} else {
		return A2(
			$elm$core$Maybe$map,
			function (n50) {
				...
				switch (_v1.$) {
					case 'RuleOfThumb':
						return n50 / 20;
					case 'SAP2012':
						return (n50 * $author$project$Main$windFactor($author$project$Main$SAP2012)) * $author$project$Main$shelterMultiplier(m.shelterFactor);
					default:
						return (n50 * $author$project$Main$windFactor($author$project$Main$HEM)) * $author$project$Main$shelterMultiplier(m.shelterFactor);
				}
			},
																					...
																					return A2(
																						...
																						function (ti) {
																							return A2(
																								...
																								function (to_) {
																									var fp = tfa / nf;
																									var w = $elm$core$Basics$sqrt(fp);
																									var perimeter = 4 * w;
																									var roofGeom = function () {
																										var _v0 = m.roofType;
																										if (_v0.$ === 'FlatRoof') {
																											return {gableArea: 0, ridgeHeight: 0, roofArea: fp, vaultedVolume: 0};
																										} else {
																											var pitch = $author$project$Main$toRad(
																												A3(
																											...
																											var hr = (w / 2) * $elm$core$Basics$tan(pitch);
																											return {
																												gableArea: (fp * $elm$core$Basics$tan(pitch)) / 2,
																												ridgeHeight: hr,
																												roofArea: fp / $elm$core$Basics$cos(pitch),
																												vaultedVolume: ((fp * w) * $elm$core$Basics$tan(pitch)) / 4
																											};
																										}
																									}();
																									var gableArea = roofGeom.gableArea;
																									var ridgeHeight = roofGeom.ridgeHeight;
																									var roofArea = roofGeom.roofArea;
																									var vaultedVolume = roofGeom.vaultedVolume;
																									var eaveHeight = nf * fh;
																									var rectWallArea = perimeter * eaveHeight;
																									var totalWallArea = rectWallArea + gableArea;
																									var glazingArea = totalWallArea * (gp / 100);
																									var opaqueWallArea = totalWallArea - glazingArea;
																									var totalExposedArea = ((opaqueWallArea + glazingArea) + roofArea) + fp;
																									var htb = yf * totalExposedArea;
																									var volume = (fp * eaveHeight) + vaultedVolume;
																									var deltaT = ti - to_;
																									var qBridges = htb * deltaT;
																									var qFloor = (fu * fp) * deltaT;
																									var qGlazing = (gu * glazingArea) * deltaT;
																									var qRoof = (ru * roofArea) * deltaT;
																									var qVent = ((0.33 * ach) * volume) * deltaT;
																									var qWalls = (wu * opaqueWallArea) * deltaT;
																									var qTotal = ((((qWalls + qGlazing) + qRoof) + qFloor) + qVent) + qBridges;
																									return {deltaT: deltaT, effectiveAch: ach, footprint: fp, gableArea: gableArea, glazingArea: glazingArea, htb: htb, opaqueWallArea: opaqueWallArea, qBridges: qBridges, qFloor: qFloor, qGlazing: qGlazing, qRoof: qRoof, qTotal: qTotal, qVent: qVent, qWalls: qWalls, ridgeHeight: ridgeHeight, roofArea: roofArea, totalExposedArea: totalExposedArea, volume: volume};
																								},
																								$elm$core$String$toFloat(m.tempOut));
																						},
																						$elm$core$String$toFloat(m.tempIn));
																				},
																				A2(
																					$elm$core$Maybe$andThen,
																					function (a) {
																						return (a >= 0) ? $elm$core$Maybe$Just(a) : $elm$core$Maybe$Nothing;
																					},
																					$author$project$Main$effectiveACH(m)));
																		},
																		$elm$core$String$toFloat(m.yFactor));
																},
																$elm$core$String$toFloat(m.glazingU));
														},
														$elm$core$String$toFloat(m.glazingPct));
												},
												$elm$core$String$toFloat(m.floorU));
										},
										$elm$core$String$toFloat(m.roofU));
								},
								$elm$core$String$toFloat(m.wallU));
						},
						$elm$core$String$toFloat(m.floorHeight));
				},
				$elm$core$String$toFloat(m.numFloors));
		},
		$elm$core$String$toFloat(m.totalFloorArea));
};
var $elm$html$Html$div = _VirtualDom_node('div');
var $elm$html$Html$h1 = _VirtualDom_node('h1');
var $author$project$Main$SetAnnualDhwKwh = function (a) {
	return {$: 'SetAnnualDhwKwh', a: a};
};
...
var $author$project$Main$SetGlazingU = function (a) {
	return {$: 'SetGlazingU', a: a};
};
var $author$project$Main$SetHDD = function (a) {
	return {$: 'SetHDD', a: a};
};
...
var $author$project$Main$SetTempIn = function (a) {
	return {$: 'SetTempIn', a: a};
};
var $author$project$Main$SetTempOut = function (a) {
	return {$: 'SetTempOut', a: a};
};
var $author$project$Main$SetTotalFloorArea = function (a) {
	return {$: 'SetTotalFloorArea', a: a};
};
var $author$project$Main$SetWallU = function (a) {
	return {$: 'SetWallU', a: a};
};
...
var $author$project$Main$inputRow = F6(
	function (label, unit, val, msg, minVal, stepVal) {
		return A2(
			$elm$html$Html$div,
			_List_fromArray(
				[
				...
				[
					...
					_List_fromArray(
						[
							...
							$elm$html$Html$Attributes$min(minVal),
							$elm$html$Html$Attributes$step(stepVal),
							A2($elm$html$Html$Attributes$style, 'width', '80px'),
							A2($elm$html$Html$Attributes$style, 'padding', '0.3rem 0.5rem'),
							A2($elm$html$Html$Attributes$style, 'border', '1px solid #d0d0d0'),
...
var $author$project$Main$sectionLabel = function (title) {
	return A2(
		$elm$html$Html$p,
		_List_fromArray(
			[
...
var $author$project$Main$inputSection = F2(
	function (title, rows) {
		return A2(
			$elm$html$Html$div,
			_List_fromArray(
			...
			_List_fromArray(
				[
					$author$project$Main$sectionLabel(title),
		...
		return A2(
			...
			_List_fromArray(
				[
					$elm$html$Html$Events$onClick(msg),
					A2($elm$html$Html$Attributes$style, 'flex', '1'),
					A2($elm$html$Html$Attributes$style, 'padding', '0.35rem 0.5rem'),
...
var $author$project$Main$modeToggle2 = F2(
	function (_v0, _v1) {
		var lA = _v0.a;
		var aA = _v0.b;
		var mA = _v0.c;
		...
		return A2(
			...
			_List_fromArray(
				[
					A3($author$project$Main$toggleBtn, lA, aA, mA),
					A3($author$project$Main$toggleBtn, lB, aB, mB)
				]));
	});
var $author$project$Main$roofSection = function (m) {
	return A2(
		$elm$html$Html$div,
		_List_fromArray(
			[
		...
		_List_fromArray(
			[
				$author$project$Main$sectionLabel('Roof'),
				A2(
				$author$project$Main$modeToggle2,
				_Utils_Tuple3(
					'Flat',
					_Utils_eq(m.roofType, $author$project$Main$FlatRoof),
					$author$project$Main$SetRoofType($author$project$Main$FlatRoof)),
				_Utils_Tuple3(
					'Vaulted / pitched',
					_Utils_eq(m.roofType, $author$project$Main$VaultedRoof),
					$author$project$Main$SetRoofType($author$project$Main$VaultedRoof))),
				function () {
				var _v0 = m.roofType;
				if (_v0.$ === 'FlatRoof') {
					return $elm$html$Html$text('');
				} else {
					return A6($author$project$Main$inputRow, 'Pitch angle', '°', m.pitchAngle, $author$project$Main$SetPitchAngle, '1', '1');
				}
			}()
			]));
};
...
var $author$project$Main$SetFloorCovering = function (a) {
	return {$: 'SetFloorCovering', a: a};
};
var $author$project$Main$SetFlowTemp = function (a) {
	return {$: 'SetFlowTemp', a: a};
};
...
var $author$project$Main$ufhSection = function (m) {
	return A2(
		...
		_List_fromArray(
			[
				$author$project$Main$sectionLabel('Underfloor Heating + ASHP'),
				A6($author$project$Main$inputRow, 'Heated floor area', 'm²', m.heatedFloorArea, $author$project$Main$SetHeatedFloorArea, '0', '5'),
				A2(
				$elm$html$Html$div,
				_List_fromArray(
					[
				...
				_List_fromArray(
					[
						...
						_List_fromArray(
							[
								$elm$html$Html$text('Floor covering')
							])),
						A2(
						$elm$html$Html$div,
						_List_fromArray(
						...
						_List_fromArray(
							[
								A3(
								$author$project$Main$radioRow,
								$author$project$Main$floorLabel($author$project$Main$Tile),
								_Utils_eq(m.floorCovering, $author$project$Main$Tile),
								$author$project$Main$SetFloorCovering($author$project$Main$Tile)),
								A3(
								$author$project$Main$radioRow,
								$author$project$Main$floorLabel($author$project$Main$Wood),
								_Utils_eq(m.floorCovering, $author$project$Main$Wood),
								$author$project$Main$SetFloorCovering($author$project$Main$Wood)),
								A3(
								$author$project$Main$radioRow,
								$author$project$Main$floorLabel($author$project$Main$Carpet),
								_Utils_eq(m.floorCovering, $author$project$Main$Carpet),
								$author$project$Main$SetFloorCovering($author$project$Main$Carpet))
							]))
					])),
				A6($author$project$Main$inputRow, 'Flow temperature', '°C', m.flowTemp, $author$project$Main$SetFlowTemp, '20', '1'),
...
var $author$project$Main$SetN50Method = function (a) {
	return {$: 'SetN50Method', a: a};
};
var $author$project$Main$SetShelterFactor = function (a) {
	return {$: 'SetShelterFactor', a: a};
};
var $elm$core$Basics$neq = _Utils_notEqual;
var $author$project$Main$shelterLabel = function (s) {
	switch (s.$) {
		case 'Exposed':
			return 'Exposed (×1.0)';
		case 'Slight':
			return 'Slight — 1 side (×0.9)';
		case 'Moderate':
			return 'Moderate — 2 sides (×0.8)';
		default:
			return 'Heavy — 3+ sides (×0.7)';
	}
};
var $author$project$Main$n50Inputs = function (m) {
	return A2(
		$elm$html$Html$div,
		_List_Nil,
		_List_fromArray(
			[
				A6($author$project$Main$inputRow, 'n50 (blower door)', 'ACH@50Pa', m.n50, $author$project$Main$SetN50, '0', '0.5'),
				A2(
				$elm$html$Html$div,
				_List_fromArray(
					[
				...
				_List_fromArray(
					[
						...
						_List_fromArray(
							[
								$elm$html$Html$text('Conversion method')
							])),
						A2(
						$elm$html$Html$div,
						_List_fromArray(
						...
						_List_fromArray(
							[
								...
								$author$project$Main$radioRow,
								'÷ 20  (rule of thumb)',
								_Utils_eq(m.n50Method, $author$project$Main$RuleOfThumb),
								$author$project$Main$SetN50Method($author$project$Main$RuleOfThumb)),
								A3(
								$author$project$Main$radioRow,
								'SAP 2012  (e = 0.07)',
								_Utils_eq(m.n50Method, $author$project$Main$SAP2012),
								$author$project$Main$SetN50Method($author$project$Main$SAP2012)),
								A3(
								$author$project$Main$radioRow,
								'Home Energy Model  (e = 0.079)',
								_Utils_eq(m.n50Method, $author$project$Main$HEM),
								$author$project$Main$SetN50Method($author$project$Main$HEM))
							]))
					])),
				(!_Utils_eq(m.n50Method, $author$project$Main$RuleOfThumb)) ? A2(
				$elm$html$Html$div,
				_List_fromArray(
					[
						A2($elm$html$Html$Attributes$style, 'margin-top', '0.4rem')
				...
				_List_fromArray(
					[
						...
						_List_fromArray(
							[
								$elm$html$Html$text('Shelter factor')
							])),
						A2(
						$elm$html$Html$div,
						_List_fromArray(
						...
						_List_fromArray(
							[
								A3(
								$author$project$Main$radioRow,
								$author$project$Main$shelterLabel($author$project$Main$Exposed),
								_Utils_eq(m.shelterFactor, $author$project$Main$Exposed),
								$author$project$Main$SetShelterFactor($author$project$Main$Exposed)),
								A3(
								$author$project$Main$radioRow,
								$author$project$Main$shelterLabel($author$project$Main$Slight),
								_Utils_eq(m.shelterFactor, $author$project$Main$Slight),
								$author$project$Main$SetShelterFactor($author$project$Main$Slight)),
								A3(
								$author$project$Main$radioRow,
								$author$project$Main$shelterLabel($author$project$Main$Moderate),
								_Utils_eq(m.shelterFactor, $author$project$Main$Moderate),
								$author$project$Main$SetShelterFactor($author$project$Main$Moderate)),
								A3(
								$author$project$Main$radioRow,
								$author$project$Main$shelterLabel($author$project$Main$Heavy),
								_Utils_eq(m.shelterFactor, $author$project$Main$Heavy),
								$author$project$Main$SetShelterFactor($author$project$Main$Heavy))
							]))
					])) : $elm$html$Html$text('')
			]));
};
var $author$project$Main$ventilationSection = function (m) {
	return A2(
		$elm$html$Html$div,
		_List_fromArray(
			[
		...
		_List_fromArray(
			[
				$author$project$Main$sectionLabel('Airtightness (natural ventilation)'),
				A2(
				$author$project$Main$modeToggle2,
				_Utils_Tuple3(
					'Direct ACH',
					_Utils_eq(m.ventMode, $author$project$Main$DirectACH),
					$author$project$Main$SetVentMode($author$project$Main$DirectACH)),
				_Utils_Tuple3(
					'n50 blower door',
					_Utils_eq(m.ventMode, $author$project$Main$FromN50),
					$author$project$Main$SetVentMode($author$project$Main$FromN50))),
				function () {
				var _v0 = m.ventMode;
				if (_v0.$ === 'DirectACH') {
					return A6($author$project$Main$inputRow, 'Air change rate', 'ACH', m.ach, $author$project$Main$SetACH, '0', '0.1');
				} else {
					return $author$project$Main$n50Inputs(m);
				}
			}()
			]));
};
var $author$project$Main$SetYFactor = function (a) {
	return {$: 'SetYFactor', a: a};
};
var $author$project$Main$presetBtn = F3(
	function (label, val, current) {
		return A2(
			$elm$html$Html$button,
			_List_fromArray(
				[
					$elm$html$Html$Events$onClick(
					$author$project$Main$SetYFactor(val)),
					A2($elm$html$Html$Attributes$style, 'font-size', '0.75rem'),
					...
					$elm$html$Html$Attributes$style,
					'border',
					'1px solid ' + (_Utils_eq(current, val) ? '#1a1a2e' : '#ccc')),
					A2(
					$elm$html$Html$Attributes$style,
					'background',
					_Utils_eq(current, val) ? '#1a1a2e' : 'transparent'),
					A2(
					$elm$html$Html$Attributes$style,
					'color',
					_Utils_eq(current, val) ? '#fff' : '#666')
				]),
			_List_fromArray(
				[
					$elm$html$Html$text(label)
...
var $author$project$Main$yFactorSection = function (m) {
	return A2(
		$elm$html$Html$div,
		_List_fromArray(
			[
		...
		_List_fromArray(
			[
				$author$project$Main$sectionLabel('Thermal Bridges'),
				A6($author$project$Main$inputRow, 'y-factor', 'W/m²K', m.yFactor, $author$project$Main$SetYFactor, '0', '0.01'),
				A2(
				$elm$html$Html$div,
				_List_fromArray(
					[
						...
						A2($elm$html$Html$Attributes$style, 'margin-top', '0.3rem')
					]),
				_List_fromArray(
					[
						A3($author$project$Main$presetBtn, 'Accredited details (0.08)', '0.08', m.yFactor),
						A3($author$project$Main$presetBtn, 'No accredited details (0.15)', '0.15', m.yFactor),
						A3($author$project$Main$presetBtn, 'Passivhaus (0.04)', '0.04', m.yFactor)
					]))
			]));
};
var $author$project$Main$inputsPanel = function (m) {
	return A2(
		$elm$html$Html$div,
		_List_Nil,
		_List_fromArray(
			[
				...
				_List_fromArray(
					[
						A6($author$project$Main$inputRow, 'Total floor area', 'm²', m.totalFloorArea, $author$project$Main$SetTotalFloorArea, '0', '5'),
						A6($author$project$Main$inputRow, 'Number of floors', '', m.numFloors, $author$project$Main$SetNumFloors, '1', '0.5'),
						A6($author$project$Main$inputRow, 'Floor-to-ceiling height', 'm', m.floorHeight, $author$project$Main$SetFloorHeight, '0.1', '0.1')
					])),
				$author$project$Main$roofSection(m),
				A2(
				$author$project$Main$inputSection,
				'Fabric U-values',
				_List_fromArray(
					[
						A6($author$project$Main$inputRow, 'Walls', 'W/m²K', m.wallU, $author$project$Main$SetWallU, '0', '0.01'),
						A6($author$project$Main$inputRow, 'Roof', 'W/m²K', m.roofU, $author$project$Main$SetRoofU, '0', '0.01'),
						A6($author$project$Main$inputRow, 'Floor', 'W/m²K', m.floorU, $author$project$Main$SetFloorU, '0', '0.01')
					])),
				A2(
				$author$project$Main$inputSection,
				'Glazing',
				...
				_List_fromArray(
					[
						A6($author$project$Main$inputRow, 'Internal (heating)', '°C', m.tempIn, $author$project$Main$SetTempIn, '', '1'),
						A6($author$project$Main$inputRow, 'External (winter)', '°C', m.tempOut, $author$project$Main$SetTempOut, '', '1'),
						A6($author$project$Main$inputRow, 'External (summer)', '°C', m.summerTempOut, $author$project$Main$SetSummerTempOut, '', '1')
					])),
				$author$project$Main$ufhSection(m),
				A2(
				$author$project$Main$inputSection,
				'Annual Energy',
				_List_fromArray(
...
var $author$project$Main$estimateCop = F2(
	function (flowC, sourceC) {
		...
		var carnot = (flowC + 273.15) / lift;
		return 0.45 * carnot;
	});
var $author$project$Main$floorCoeff = function (c) {
	switch (c.$) {
		case 'Tile':
			return 6.7;
		case 'Wood':
...
var $author$project$Main$calculateHeating = F2(
	function (m, r) {
		return A2(
			$elm$core$Maybe$andThen,
			function (heatedFloorAreaM2) {
				return A2(
					$elm$core$Maybe$andThen,
					function (flowTempC) {
						return A2(
							$elm$core$Maybe$andThen,
							function (emitterDeltaTK) {
								return A2(
									$elm$core$Maybe$andThen,
									function (hddCibseDegDays) {
										return A2(
											$elm$core$Maybe$andThen,
											function (indoorTempC) {
												return A2(
													$elm$core$Maybe$andThen,
													function (outdoorTempC) {
														return A2(
															$elm$core$Maybe$andThen,
															function (gValue) {
																return A2(
																	$elm$core$Maybe$andThen,
																	function (totalFloorAreaM2) {
																		return A2(
																			$elm$core$Maybe$map,
																			function (horizIrradKwhPerM2Yr) {
																				var tauHours = $author$project$Main$thermalMassTau(m.thermalMass);
																				var specHeatLossWperK = (r.deltaT > 0) ? (r.qTotal / r.deltaT) : 0;
																				var scop = A2($author$project$Main$estimateCop, flowTempC, 7);
																				var requiredSpecificWperM2 = (heatedFloorAreaM2 > 0) ? (r.qTotal / heatedFloorAreaM2) : 0;
																				var monthlyHddFractions = $author$project$Main$hddFractionsAtBase(indoorTempC);
																				var meanWaterTempC = flowTempC - (emitterDeltaTK / 2);
																				var waterAboveIndoorK = A2($elm$core$Basics$max, 0, meanWaterTempC - indoorTempC);
																				var hddAtSetpointDegDays = $author$project$Main$hddAtBase(indoorTempC);
																				var hddAtCibseBaseDegDays = $author$project$Main$hddAtBase($author$project$Main$cibseHddBase);
																				var hddScaleFactor = (hddAtCibseBaseDegDays > 0) ? (hddAtSetpointDegDays / hddAtCibseBaseDegDays) : 1;
																				var floorKwPerM2K = $author$project$Main$floorCoeff(m.floorCovering);
																				var requiredMeanWaterTempC = indoorTempC + ((floorKwPerM2K > 0) ? (requiredSpecificWperM2 / floorKwPerM2K) : 0);
																				var requiredFlowTempC = requiredMeanWaterTempC + (emitterDeltaTK / 2);
																				var specificOutputWperM2 = floorKwPerM2K * waterAboveIndoorK;
																				var maxOutputW = specificOutputWperM2 * heatedFloorAreaM2;
																				var surfaceTempC = indoorTempC + (specificOutputWperM2 / $author$project$Main$floorSurfaceH);
																				var effectiveHddDegDays = hddCibseDegDays * hddScaleFactor;
																				var designCop = A2($author$project$Main$estimateCop, flowTempC, outdoorTempC);
																				var coveragePct = (r.qTotal > 0) ? ((maxOutputW / r.qTotal) * 100) : 0;
																				var annualSolarGainKwh = ((r.glazingArea * gValue) * horizIrradKwhPerM2Yr) * $author$project$Main$glazingVertFactor;
																				var annualInternalGainKwh = (($author$project$Main$internalGainsWperM2 * totalFloorAreaM2) * 8760) / 1000;
																				var annualTotalGainKwh = annualSolarGainKwh + annualInternalGainKwh;
																				var internalMonthlyKwh = A2(
																					$elm$core$List$map,
																					function (d) {
																						return (annualInternalGainKwh * d) / 365;
																					},
																					$author$project$Main$daysInMonth);
																				var annualHeatKwh = ((specHeatLossWperK * effectiveHddDegDays) * 24) / 1000;
																				var monthlyUsefulKwh = A4(
																					$elm$core$List$map3,
																					F3(
																						function (hddFrac, pvFrac, internalKwh) {
																							var grossHeatKwh = annualHeatKwh * hddFrac;
																							var gainKwh = (annualSolarGainKwh * pvFrac) + internalKwh;
																							return (grossHeatKwh < 0.001) ? 0 : A2(
																								$elm$core$Basics$min,
																								grossHeatKwh,
																								A2($author$project$Main$utilisationFactor, gainKwh / grossHeatKwh, tauHours) * gainKwh);
																						}),
																					monthlyHddFractions,
																					$author$project$Main$pvFractions,
																					internalMonthlyKwh);
																				var annualUsefulGainKwh = $elm$core$List$sum(monthlyUsefulKwh);
																				var annualNetHeatKwh = A2($elm$core$Basics$max, 0, annualHeatKwh - annualUsefulGainKwh);
																				var annualExcessGainKwh = A2($elm$core$Basics$max, 0, annualTotalGainKwh - annualUsefulGainKwh);
																				var annualElecKwh = (scop > 0) ? (annualNetHeatKwh / scop) : 0;
																				return {annualElecKwh: annualElecKwh, annualExcessGain: annualExcessGainKwh, annualHeatKwh: annualHeatKwh, annualInternalGain: annualInternalGainKwh, annualNetHeatKwh: annualNetHeatKwh, annualSolarGain: annualSolarGainKwh, annualUsefulGain: annualUsefulGainKwh, coverage: coveragePct, designCop: designCop, effectiveHdd: effectiveHddDegDays, hddFractions: monthlyHddFractions, maxOutput: maxOutputW, meanWaterTemp: meanWaterTempC, requiredFlowTemp: requiredFlowTempC, scop: scop, specificOutput: specificOutputWperM2, surfaceTemp: surfaceTempC};
																			},
									...
									$elm$core$String$toFloat(m.hdd));
							},
							$elm$core$String$toFloat(m.emitterDeltaT));
					},
					$elm$core$String$toFloat(m.flowTemp));
			},
...
var $author$project$Main$chartLegend = A2(
	...
	_List_fromArray(
		[
			...
			A2($author$project$Main$legendSwatch, '#444466', 'EV')
		]));
				...
				_List_fromArray(
					[
						...
						$elm$core$String$fromFloat(
							chartH * (1 - (v / A2($elm$core$Basics$max, maxVal, 0.001)))) + 'px'),
...
var $author$project$Main$coolingChartSection = function (rows) {
	var maxC = A2(
		$elm$core$Maybe$withDefault,
		0,
	...
	return (maxC < 0.01) ? A2(
		...
		_List_fromArray(
			[
				$elm$html$Html$text('Cooling demand: ~0 across all months (gains absorbed by fabric).')
			])) : A2(
		$elm$html$Html$div,
		_List_fromArray(
			[
		...
		_List_fromArray(
			[
				...
				_List_fromArray(
					[
						$elm$html$Html$text('Monthly — Cooling Demand (excess gain after free cooling)')
					])),
				A2(
				$elm$html$Html$div,
				_List_fromArray(
				...
				_List_fromArray(
					[
						A2($author$project$Main$legendSwatch, '#3b82c4', 'Cooling demand')
					])),
				A4(
				$author$project$Main$singleBarChart,
				rows,
				function ($) {
					return $.coolingKwh;
				},
				...
				return A2(
					...
					_List_fromArray(
						[
							...
							$elm$html$Html$Attributes$style,
							'width',
							$elm$core$String$fromFloat(barW) + 'px'),
							A2(
							$elm$html$Html$Attributes$style,
							'height',
...
var $author$project$Main$monthlyBreakdown = F4(
	function (m, r, u, pv) {
		...
		var internalDaily = (($author$project$Main$internalGainsWperM2 * tfa) * 24) / 1000;
		var coolTarget = A2(
			$elm$core$Maybe$withDefault,
			$author$project$Main$coolingSetpoint,
			$elm$core$String$toFloat(m.tempIn));
		var batteryKwh = A2(
			$elm$core$Maybe$withDefault,
			0,
			$elm$core$String$toFloat(m.batteryKwh));
		var annualHousehold = A2(
			$elm$core$Maybe$withDefault,
			0,
			$elm$core$String$toFloat(m.householdElecKwh));
		var householdDaily = annualHousehold / 365;
		var annualEvMiles = A2(
			$elm$core$Maybe$withDefault,
			0,
			$elm$core$String$toFloat(m.evMilesPerYear));
		var milesPerDay = annualEvMiles / 365;
		var annualDhw = A2(
			$elm$core$Maybe$withDefault,
			0,
			$elm$core$String$toFloat(m.annualDhwKwh));
		return A6(
			$elm$core$List$map5,
			F5(
				function (name, _v0, dl, days, avgT) {
					var hddF = _v0.a;
					var pvF = _v0.b;
					var dhwF = _v0.c;
					...
					var dayHours = dl;
					var freeCoolDay = ((ua * A2($elm$core$Basics$max, 0, coolTarget - dayT)) * dayHours) / 1000;
					var dayFrac = dl / 24;
					var hpDay = (netElec * dayFrac) / days;
					var hpNight = (netElec * (1 - dayFrac)) / days;
					var coolingNightDemand = A2($elm$core$Basics$max, 0, internalNight - freeCoolNight);
					var coolingDayDemand = A2($elm$core$Basics$max, 0, (excessSolarDay + internalDay) - freeCoolDay);
					var coolingDaily = coolingDayDemand + coolingNightDemand;
					var coolNightElec = coolingNightDemand / $author$project$Main$coolingScop;
					var coolDayElec = coolingDayDemand / $author$project$Main$coolingScop;
					var dayLoad = (hpDay + coolDayElec) + householdDay;
					var dayShortfall = A2($elm$core$Basics$max, 0, dayLoad - pvDaily);
					var pvSurplus = A2($elm$core$Basics$max, 0, pvDaily - dayLoad);
					var pvToEvDay = A2($elm$core$Basics$min, pvSurplus, evDaily);
					var evRemaining = evDaily - pvToEvDay;
				...
				return A2(
					...
					function (u) {
						return A2(
							$elm$core$Maybe$map,
							function (pv) {
								return _Utils_Tuple3(r, u, pv);
							},
							A2($author$project$Main$calculatePv, model, u));
					},
					A2($author$project$Main$calculateHeating, model, r));
			},
			maybeR);
		if (_v0.$ === 'Nothing') {
			return $elm$html$Html$text('');
...
var $author$project$Main$blindsFactor = 0.6;
var $author$project$Main$peakHorizFromAnnual = function (annual) {
	return A2($elm$core$Basics$min, 950, 0.85 * annual);
};
var $author$project$Main$calculateCooling = F2(
	function (m, r) {
		return A2(
			$elm$core$Maybe$andThen,
			function (toS) {
						...
						return A2(
							...
							function (tfa) {
								return A2(
									$elm$core$Maybe$map,
									function (horizIrr) {
										var uaVent = (r.deltaT > 0) ? (r.qVent / r.deltaT) : 0;
										var tIn = $author$project$Main$coolingSetpoint;
										var qInternal = $author$project$Main$internalGainsWperM2 * tfa;
										var peakHoriz = $author$project$Main$peakHorizFromAnnual(horizIrr);
										var qSolar = (((peakHoriz * r.glazingArea) * gVal) * $author$project$Main$glazingVertFactor) * $author$project$Main$blindsFactor;
										var fabricW = (((r.qWalls + r.qGlazing) + r.qRoof) + r.qFloor) + r.qBridges;
										var uaFabric = (r.deltaT > 0) ? (fabricW / r.deltaT) : 0;
										var dT = toS - tIn;
										var qFabric = uaFabric * dT;
										var qVent = uaVent * dT;
										var qTotal = A2($elm$core$Basics$max, 0, ((qFabric + qVent) + qInternal) + qSolar);
										return {deltaT: dT, peakHoriz: peakHoriz, qFabric: qFabric, qInternal: qInternal, qSolar: qSolar, qTotal: qTotal, qVent: qVent};
									},
									$elm$core$String$toFloat(m.pvIrradiation));
							},
							$elm$core$String$toFloat(m.totalFloorArea));
			...
			$elm$core$String$toFloat(m.summerTempOut));
	});
		...
		return A2(
			...
			_List_fromArray(
				[
					...
					A2($elm$html$Html$Attributes$style, 'border-radius', '10px'),
					A2($elm$html$Html$Attributes$style, 'padding', '1rem 1.25rem'),
...
var $author$project$Main$detailRow = F3(
	function (label, val, unit) {
		return A2(
			$elm$html$Html$div,
			_List_fromArray(
				[
			...
			_List_fromArray(
				[
					...
					_List_fromArray(
						[
							$elm$html$Html$text(
							$elm$core$String$isEmpty(unit) ? val : (val + ('\u00A0' + unit)))
						]))
				]));
	});
...
var $author$project$Main$coolingCard = function (c) {
	return A3(
		...
		_List_fromArray(
			[
				...
				'ΔT (out − in)',
				$author$project$Main$fmt1(c.deltaT),
				'K'),
				A3(
				$author$project$Main$detailRow,
				'Peak horiz. irrad.',
				$elm$core$String$fromInt(
					$elm$core$Basics$round(c.peakHoriz)),
				'W/m²'),
				A2(
				$elm$html$Html$hr,
				_List_fromArray(
				...
				'Fabric gain',
				$elm$core$String$fromInt(
					$elm$core$Basics$round(c.qFabric)),
				'W'),
				A3(
				$author$project$Main$detailRow,
				'Ventilation gain',
				$elm$core$String$fromInt(
					$elm$core$Basics$round(c.qVent)),
				'W'),
				A3(
				$author$project$Main$detailRow,
				'Internal gains',
				$elm$core$String$fromInt(
					$elm$core$Basics$round(c.qInternal)),
				'W'),
				A3(
				$author$project$Main$detailRow,
				'Solar (peak through glazing)',
				$elm$core$String$fromInt(
					$elm$core$Basics$round(c.qSolar)),
				'W'),
				A2(
				$elm$html$Html$hr,
				_List_fromArray(
						...
						_List_fromArray(
							[
								...
								_List_fromArray(
									[
										$elm$html$Html$text(
										$author$project$Main$fmt2(c.qTotal / 1000) + ' kW')
									])),
								A2(
								$elm$html$Html$div,
								_List_fromArray(
									[
										A2($elm$html$Html$Attributes$style, 'font-size', '0.78rem'),
										A2($elm$html$Html$Attributes$style, 'color', '#888'),
										A2($elm$html$Html$Attributes$style, 'margin-top', '0.2rem')
									]),
								_List_fromArray(
									[
										$elm$html$Html$text(
										$elm$core$String$fromInt(
											$elm$core$Basics$round(c.qTotal)) + ' W')
									]))
							]))
					])),
				A2(
				...
				_List_fromArray(
					[
						$elm$html$Html$text('Setpoint 25 °C. Internal gains 4 W/m². Solar assumes internal blinds (×0.6).')
					]))
			]));
};
			...
			_Utils_ap(
				_List_fromArray(
					[
						...
						$author$project$Main$detailRow,
						'Footprint',
						$author$project$Main$fmt1(r.footprint),
						'm²'),
						A3(
						$author$project$Main$detailRow,
						'Opaque walls',
						$author$project$Main$fmt1(r.opaqueWallArea),
						'm²'),
						A3(
						$author$project$Main$detailRow,
						'Glazing',
						$author$project$Main$fmt1(r.glazingArea),
						'm²'),
						A3(
						$author$project$Main$detailRow,
						'Roof',
						$author$project$Main$fmt1(r.roofArea),
						'm²')
					]),
				_Utils_ap(
					function () {
						var _v0 = model.roofType;
						if (_v0.$ === 'FlatRoof') {
							return _List_Nil;
						} else {
							return _List_fromArray(
								[
									...
									$author$project$Main$detailRow,
									'  Gable walls',
									$author$project$Main$fmt1(r.gableArea),
									'm²'),
									A3(
									$author$project$Main$detailRow,
									'  Ridge height',
									$author$project$Main$fmt2(r.ridgeHeight),
									'm')
								]);
						}
					}(),
					_List_fromArray(
						[
							...
							$author$project$Main$detailRow,
							'Total exposed area',
							$author$project$Main$fmt1(r.totalExposedArea),
							'm²'),
							A3(
							$author$project$Main$detailRow,
							'Thermal bridge coeff. (y × area)',
							$author$project$Main$fmt1(r.htb),
							'W/K'),
							A3(
							$author$project$Main$detailRow,
							'Volume',
							$author$project$Main$fmt1(r.volume),
							'm³'),
							A3(
							$author$project$Main$detailRow,
							'Temperature difference',
							$author$project$Main$fmt1(r.deltaT),
							'K'),
							A2(
							$elm$html$Html$hr,
							_List_fromArray(
							...
							_List_Nil),
							function () {
							var _v1 = model.ventMode;
							if (_v1.$ === 'DirectACH') {
								return A3(
									$author$project$Main$detailRow,
									'Effective ACH',
									$author$project$Main$fmt2(r.effectiveAch),
									'ACH');
							} else {
								return A2(
									$elm$html$Html$div,
									...
									_List_fromArray(
										[
											A3($author$project$Main$detailRow, 'n50', model.n50, 'ACH@50Pa'),
											A3(
											$author$project$Main$detailRow,
											'Effective ACH',
											$author$project$Main$fmt2(r.effectiveAch),
...
var $author$project$Main$lossRow = F4(
	function (label, watts, total, colour) {
		var pct = (total > 0) ? A2($elm$core$Basics$max, 0, (watts / total) * 100) : 0;
		return A2(
			$elm$html$Html$div,
			_List_fromArray(
				[
				...
				[
					...
					_List_fromArray(
						[
							...
							A2($elm$html$Html$Attributes$style, 'justify-content', 'space-between'),
							A2($elm$html$Html$Attributes$style, 'font-size', '0.88rem'),
							A2($elm$html$Html$Attributes$style, 'margin-bottom', '0.25rem')
						]),
					_List_fromArray(
						[
							A2(
							...
							_List_fromArray(
								[
									$elm$html$Html$text(
									$elm$core$String$fromInt(
										$elm$core$Basics$round(watts)) + (' W (' + ($author$project$Main$fmt1(pct) + '%)')))
								]))
						])),
					A2(
					$elm$html$Html$div,
					_List_fromArray(
						[
							A2($elm$html$Html$Attributes$style, 'background', '#e0e4f0'),
							A2($elm$html$Html$Attributes$style, 'border-radius', '4px'),
						...
						[
							...
							_List_fromArray(
								[
									...
									A2($elm$html$Html$Attributes$style, 'border-radius', '4px'),
									A2($elm$html$Html$Attributes$style, 'transition', 'width 0.2s ease')
								]),
							_List_Nil)
						]))
				]));
	});
var $author$project$Main$heatLossCard = function (r) {
	return A3(
		$author$project$Main$card,
		'#f5f7ff',
		'Heat Loss',
		_List_fromArray(
			[
				A4($author$project$Main$lossRow, 'Walls', r.qWalls, r.qTotal, '#4a6fa5'),
				A4($author$project$Main$lossRow, 'Glazing', r.qGlazing, r.qTotal, '#e07b39'),
				A4($author$project$Main$lossRow, 'Roof', r.qRoof, r.qTotal, '#5a9e6f'),
				A4($author$project$Main$lossRow, 'Floor', r.qFloor, r.qTotal, '#c8a020'),
				A4($author$project$Main$lossRow, 'Infiltration', r.qVent, r.qTotal, '#9b5ea2'),
				A4($author$project$Main$lossRow, 'Thermal bridges', r.qBridges, r.qTotal, '#c44569'),
				A2(
				$elm$html$Html$hr,
				_List_fromArray(
					[
						A2($elm$html$Html$Attributes$style, 'border', 'none'),
						A2($elm$html$Html$Attributes$style, 'border-top', '2px solid #1a1a2e'),
						A2($elm$html$Html$Attributes$style, 'margin', '0.75rem 0')
					]),
				_List_Nil),
				A2(
				$elm$html$Html$div,
					...
					[
						...
						_List_fromArray(
							[
								...
								_List_fromArray(
									[
										A2($elm$html$Html$Attributes$style, 'font-size', '1.75rem'),
										A2($elm$html$Html$Attributes$style, 'font-weight', '700'),
										A2($elm$html$Html$Attributes$style, 'color', '#1a1a2e'),
										A2($elm$html$Html$Attributes$style, 'line-height', '1')
									]),
								_List_fromArray(
									[
										$elm$html$Html$text(
										$author$project$Main$fmt2(r.qTotal / 1000) + ' kW')
									])),
								A2(
								$elm$html$Html$div,
								_List_fromArray(
								...
								_List_fromArray(
									[
										$elm$html$Html$text(
										$elm$core$String$fromInt(
											$elm$core$Basics$round(r.qTotal)) + ' W')
									]))
							]))
					]))
			]));
		...
		return A3(
			...
			_List_fromArray(
				[
					...
					'Cooling demand',
					$elm$core$String$fromInt(
						$elm$core$Basics$round(annualCoolKwh)),
					'kWh/yr'),
					A2(
					$elm$html$Html$hr,
					_List_fromArray(
					...
					'Electricity (heating)',
					$elm$core$String$fromInt(
						$elm$core$Basics$round(u.annualElecKwh)),
					'kWh/yr'),
					A3(
					$author$project$Main$detailRow,
					'Electricity (hot water, SCOP ' + ($author$project$Main$fmt1($author$project$Main$dhwScop) + ')'),
					$elm$core$String$fromInt(
						$elm$core$Basics$round(annualDhwElec)),
					'kWh/yr'),
					A3(
					$author$project$Main$detailRow,
					'Electricity (cooling, SCOP ' + ($author$project$Main$fmt1($author$project$Main$coolingScop) + ')'),
					$elm$core$String$fromInt(
						$elm$core$Basics$round(annualCoolElec)),
					'kWh/yr'),
					A3(
					$author$project$Main$detailRow,
					'Electricity (household)',
					$elm$core$String$fromInt(
						$elm$core$Basics$round(annualHouseholdKwh)),
					'kWh/yr'),
					A3(
					$author$project$Main$detailRow,
					'Electricity (EV)',
					$elm$core$String$fromInt(
						$elm$core$Basics$round(annualEvKwh)),
					'kWh/yr'),
					A3(
					$author$project$Main$detailRow,
					'Electricity (total)',
					$elm$core$String$fromInt(
						$elm$core$Basics$round(totalElec)),
					'kWh/yr'),
					A2(
					$elm$html$Html$hr,
					_List_fromArray(
...
var $author$project$Main$ufhCard = F2(
	function (model, u) {
		var surfaceWarn = u.surfaceTemp > 29;
		var coverageColor = (u.coverage >= 100) ? '#2e7d32' : ((u.coverage >= 85) ? '#c77700' : '#c62828');
		return A3(
			$author$project$Main$card,
			'#f5f7ff',
			'UFH + Heat Pump',
			_List_fromArray(
				[
					...
					$author$project$Main$detailRow,
					'Mean water temp',
					$author$project$Main$fmt1(u.meanWaterTemp),
					'°C'),
					A3(
					$author$project$Main$detailRow,
					'Specific output',
					$author$project$Main$fmt1(u.specificOutput),
					'W/m²'),
					A3(
					$author$project$Main$detailRow,
					'Max UFH output',
					$elm$core$String$fromInt(
						$elm$core$Basics$round(u.maxOutput)),
					'W'),
					A2(
					$elm$html$Html$div,
					_List_fromArray(
					...
					_List_fromArray(
						[
							$elm$html$Html$text('Coverage vs heat loss'),
							A2(
							$elm$html$Html$span,
							_List_fromArray(
								[
									A2($elm$html$Html$Attributes$style, 'color', coverageColor),
									A2($elm$html$Html$Attributes$style, 'font-variant-numeric', 'tabular-nums')
								]),
							_List_fromArray(
								[
									$elm$html$Html$text(
									$author$project$Main$fmt1(u.coverage) + ' %')
								]))
						])),
					A3(
					$author$project$Main$detailRow,
					'Required flow temp',
					$author$project$Main$fmt1(u.requiredFlowTemp),
					'°C'),
					A3(
					$author$project$Main$detailRow,
					'Floor surface temp',
					$author$project$Main$fmt1(u.surfaceTemp),
					'°C'),
					surfaceWarn ? A2(
					$elm$html$Html$p,
					_List_fromArray(
						[
							...
							A2($elm$html$Html$Attributes$style, 'margin-top', '0.4rem'),
							A2($elm$html$Html$Attributes$style, 'margin-bottom', '0.2rem')
						]),
					_List_fromArray(
						[
							$elm$html$Html$text('⚠ Floor surface > 29 °C (comfort limit). Lower flow temp or add emitter area.')
						])) : $elm$html$Html$text(''),
					A2(
					$elm$html$Html$hr,
					_List_fromArray(
					...
					A3(
					$author$project$Main$detailRow,
					'Design COP (source ' + (model.tempOut + ' °C)'),
					$author$project$Main$fmt2(u.designCop),
					''),
					A3(
					$author$project$Main$detailRow,
					'SCOP est. (source 7 °C)',
					$author$project$Main$fmt2(u.scop),
					'')
				]));
	});
var $author$project$Main$resultsPanel = F2(
	function (model, maybeR) {
		if (maybeR.$ === 'Nothing') {
			return A2(
				$elm$html$Html$div,
				_List_fromArray(
					[
						A2($elm$html$Html$Attributes$style, 'background', '#fff8e1'),
						A2($elm$html$Html$Attributes$style, 'border', '1px solid #ffe082'),
						A2($elm$html$Html$Attributes$style, 'border-radius', '10px'),
						A2($elm$html$Html$Attributes$style, 'padding', '1rem'),
						A2($elm$html$Html$Attributes$style, 'font-size', '0.88rem'),
						A2($elm$html$Html$Attributes$style, 'color', '#7a6000')
					]),
				_List_fromArray(
					[
						$elm$html$Html$text('Enter valid numbers in all fields to see results.')
					]));
		} else {
			var r = maybeR.a;
			return A2(
				...
				_List_fromArray(
					[
						A2($author$project$Main$geometryCard, model, r),
						$author$project$Main$heatLossCard(r),
						function () {
						var _v1 = A2($author$project$Main$calculateCooling, model, r);
						if (_v1.$ === 'Just') {
							var c = _v1.a;
							return $author$project$Main$coolingCard(c);
						} else {
							return $elm$html$Html$text('');
						}
					}(),
						function () {
						var _v2 = A2($author$project$Main$calculateHeating, model, r);
						if (_v2.$ === 'Just') {
							var u = _v2.a;
							return A2($author$project$Main$ufhCard, model, u);
						} else {
							return $elm$html$Html$text('');
						}
					}(),
						...
						if (_v3.$ === 'Just') {
							...
							var annualCoolElec = sumOver(
								function (row) {
									return row.dayCoolKwh + row.nightCoolKwh;
								});
							var annualBatteryKwh = sumOver(
								function (row) {
	...
	return A2(
		...
		_List_fromArray(
			[
				...
				A2($elm$html$Html$Attributes$style, 'padding', '0 1.5rem'),
				A2($elm$html$Html$Attributes$style, 'color', '#1a1a2e')
			]),
		_List_fromArray(
			[
				...
				_List_fromArray(
					[
						...
						A2($elm$html$Html$Attributes$style, 'gap', '2.5rem'),
						A2($elm$html$Html$Attributes$style, 'align-items', 'start')
					]),
				_List_fromArray(
					[
						$author$project$Main$inputsPanel(model),
						A2(
